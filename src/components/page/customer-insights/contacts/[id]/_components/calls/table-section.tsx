...
export function CallsTableSection({ ...props }: Props) {
  ...
  useEffect(() => {
    ...
    toast.message('Loading contact calls...');

    apiClient
      .getCrispContactCalls(
        `base_contact_id=${props.baseContactId}&page=${page}&count=${perPage}`
      )
      .then((res) => {
        setResponse(res);
        toast.dismiss();
      ...
      .catch(() => {
        toast.dismiss();
        toast.error(
          'Something unexpected occured while retrieving contact calls.'
        );
      });
